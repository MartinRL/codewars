...
namespace codewars
{
    ...
    {
        public static int CalculateDaysFor(int upSpeed, int downSpeed, int desiredHeight) => upSpeed >= desiredHeight ? 1 : (int)Ceiling(Round((desiredHeight / (decimal)downSpeed - 1) / (upSpeed / (decimal)downSpeed - 1), 2));
    }
